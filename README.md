...
```
https://github.com/litetokens/java-litetokens
event_parser branch
based on：org.litetokens.core.db.Manager#sendEventLog
```


## The Following are Four Methods for Polling:
### 1.  By Contract Address:<br>
    curl https://api.shasta.litetokens.org/event/contract/TMJnJcHfdP5rhmXVkwRYb1a9A6gS46PUm6
    
    By default, its gonna return the latest 50 events. If you want to return the latest event:
    curl https://api.shasta.litetokens.org/event/contract/TMJnJcHfdP5rhmXVkwRYb1a9A6gS46PUm6?size=1
    
    if you want to return the second latest event,
    curl https://api.shasta.litetokens.org/event/contract/TMJnJcHfdP5rhmXVkwRYb1a9A6gS46PUm6?size=1&page=2
### 2.  By Contract Address and Event Name:<br>
    curl https://api.litetokens.org/event/contract/TMJnJcHfdP5rhmXVkwRYb1a9A6gS46PUm6/Notify
    
    to retrieve the latest event of the contract:
    curl https://api.litetokens.org/event/contract/TMJnJcHfdP5rhmXVkwRYb1a9A6gS46PUm6/Notify?size=1
    
    to retrieve the second latest event of the contract:
    curl https://api.litetokens.org/event/contract/TMJnJcHfdP5rhmXVkwRYb1a9A6gS46PUm6/Notify?size=1&page=2
### 3.  By Contract Address, Event Name, and Block Height:
    curl https://api.litetokens.org/event/contract/TMJnJcHfdP5rhmXVkwRYb1a9A6gS46PUm6/Notify/88
### 4.  By Transaction ID:<br>
curl https://api.litetokens.org/event/transaction/5c3747ffa94fc87a2188708a9e0758cbd01f000d3d01f6589651921930183f6a
<br>

## Request Parameters:<br>
    since: set a time stamp, default 0, returning all events after that timestamp
    EX: curl https://api.litetokens.org/event/contract/TMJnJcHfdP5rhmXVkwRYb1a9A6gS46PUm6/Notify?since=1541547888000
    
    size: number of results return. Default is 20, maximum is 200
    EX: curl https://api.litetokens.org/event/contract/TMJnJcHfdP5rhmXVkwRYb1a9A6gS46PUm6/Notify?size=10
    
    page: page number
    EX: curl https://api.litetokens.org/event/contract/TMJnJcHfdP5rhmXVkwRYb1a9A6gS46PUm6/Notify?size=10&page=2
...
```
Deploy Contract Using Litetokensweb, Litetokensbox, or directly using rpc or http call:
```
...
```
cd target
nohup java -jar litetokensgrid-0.0.1-SNAPSHOT.jar &
```
